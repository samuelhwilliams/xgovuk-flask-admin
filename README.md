...
Then initialise Flask-Admin and xgov-flask-admin appropriately, making sure to pass the GOV.UK Frontend Theme to
Flask-Admin.
...
from xgov_flask_admin import XGovFlaskAdmin, XGovFrontendTheme
...
xgov_flask_admin = XGovFlaskAdmin(app)
```

All of your SQLAlchemy model fields should derive from xgov-flask-admin's `XGovModelView`, not from Flask-Admin's
`sqla.ModelView`.
...
`xgov-flask-admin` will aim to be compatible with all supported versions of Python released from 2025, but this 
...
`xgov-flask-admin` is packaged with compiled static assets that are served directly by the extension. This is a 
...
Since `xgov-flask-admin` compiles assets from GOV.UK Frontend (and other x-gov frontend components), it will be 
...
Please comment on https://github.com/samuelhwilliams/xgov-flask-admin/issues/1 if this would be useful.

The latest release of `xgov-flask-admin` will generally track the latest release of GOV.UK Frontend, 
...
`xgov-flask-admin` uses `govuk-colour("purple")` as its brand colour on the (somewhat unfounded/unresearched) belief 
...
com/samuelhwilliams/xgov-flask-admin/issues/7 if customisation/extension is needed.
...
components used in xgov-flask-admin are not 'officially' fully supported or fully accessible, and xgov-flask-admin 
...
3. Run `flask vite build` to compile GOV.UK Frontend assets and copy them into `xgov_flask_admin/static`
...
- Use `uv version <new version #>` to update the xgov-flask-admin's version
