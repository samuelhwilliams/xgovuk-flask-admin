"""Integration tests for SelectWithSearchFilter."""
...
class TestSelectWithSearchFilterApply:
    def test_single_select_filters_query(self, db_session, filter_test_data):
        alice, _ = filter_test_data
        flt = SelectWithSearchFilter(
            Post.author_id,
            name="Author",
            predicate_builder=lambda col, value: col == int(value),
            options=lambda: [],
        )

        query = db_session.query(Post)
        filtered = flt.apply(query, str(alice.id))

        titles = sorted(p.title for p in filtered.all())
        assert titles == ["Alice 1", "Alice 2"]

    def test_multi_select_filters_query(self, db_session, filter_test_data):
